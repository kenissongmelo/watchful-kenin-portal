...
export const KeninDutyMainPage = () => {
  ...
  const handleTestConnections = async () => {
    try {
      const data = await api.testConnections();
      
      const connectionStatus = Object.entries(data).map(([provider, info]: [string, any]) => 
        `${provider.toUpperCase()}: ${info.status === 'success' ? '✅' : '❌'} ${info.message}`
      ).join('\n');
      
      alertApi.post({ 
        message: `Teste de Conexões:\n${connectionStatus}`, 
        severity: 'info' 
      });
    } catch (error) {
      alertApi.post({ message: 'Erro ao testar conexões', severity: 'error' });
    }
  };
    ...
    <div className={classes.root}>
      ...
      <Paper>
            ...
            {stats && (
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={3}>
                  <Card className={classes.statCard}>
                    <CardContent>
                      ...
                      <Typography className={classes.statNumber} color="error">
                        {stats.activeAlerts}
                      </Typography>
                ...
                <Grid item xs={12} md={3}>
                  <Card className={classes.statCard}>
                    <CardContent>
                      <PhoneIcon style={{ color: '#4caf50' }} fontSize="large" />
                            ...
                            <Box display="flex" justifyContent="space-between" alignItems="center">
                              ...
                              <Box>
                                <Chip
                                  ...
                                  size="small"
                                  color={alert.severity === 'critical' ? 'secondary' : 'default'}
                                />
                                <Chip
                                  label={alert.status}
                                  size="small"
        ...
        <TabPanel value={activeTab} index={1}>
          <div className={classes.tabContent}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
              <Typography variant="h5">Gerenciamento de Times</Typography>
              <Button
                variant="contained"
                color="primary"
                startIcon={<AddIcon />}
                      ...
                      {team.retryPolicy && (
                        <Box mb={2}>
                          ...
                          <Box display="flex" flexWrap="wrap">
                            ...
                            <Chip
                              size="small"
                              label={`${team.retryPolicy.retryInterval}min intervalo`}
                              color="secondary"
                              variant="outlined"
                              style={{ marginRight: 4, marginBottom: 4 }}
                            />
        ...
        <TabPanel value={activeTab} index={2}>
          <div className={classes.tabContent}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
              <Typography variant="h5">Gerenciamento de Alertas</Typography>
              <Button
                variant="contained"
                color="secondary"
                ...
                <TableBody>
                  {alerts.map((alert) => (
                    <TableRow key={alert.id}>
                      ...
                      <TableCell>
                        {teams.find(t => t.id === alert.teamId)?.name || alert.teamId}
                      </TableCell>
                      <TableCell>
                        <Chip
                          label={alert.severity}
                      ...
                      <TableCell>
                        <Chip
                          label={alert.status}
                          size="small"
                          color={alert.status === 'active' ? 'secondary' : 'default'}
                        />
                      </TableCell>
                      <TableCell>{alert.provider || 'Manual'}</TableCell>
                      <TableCell>
        ...
        <TabPanel value={activeTab} index={3}>
          <div className={classes.tabContent}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
              <Typography variant="h5">Histórico de Chamadas</Typography>
              <Button
                variant="outlined"
                startIcon={<RefreshIcon />}
                onClick={loadData}
                ...
                <TableBody>
                  {callAttempts.map((attempt) => (
                    <TableRow key={attempt.id}>
                      <TableCell>{attempt.memberName}</TableCell>
                      <TableCell>
                        {teams.find(t => t.id === attempt.teamId)?.name || attempt.teamId}
                      </TableCell>
                      <TableCell>{attempt.phone}</TableCell>
                      <TableCell>
                        <Chip
                          label={attempt.status}
                          size="small"
                          color={attempt.status === 'answered' ? 'primary' : 'default'}
                        />
                      </TableCell>
                      <TableCell>
                        {attempt.duration ? `${attempt.duration}s` : '-'}
                      </TableCell>
                      <TableCell>{attempt.retryCount + 1}</TableCell>
                      <TableCell>
          ...
          <div className={classes.tabContent}>
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body2" color="textSecondary" gutterBottom>
                  Sistema integrado com a API Go em localhost:8080 para gerenciamento de chamadas automáticas
                </Typography>
                ...
                <Box mt={2}>
                  ...
                  <List>
                    <ListItem>
                      <ListItemText
                        primary="Máximo de Tentativas"
                        secondary="3 tentativas por membro"
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemText
                        primary="Escalação Automática"
                        secondary="Após falha, escala para próximo membro disponível"
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemText
                        primary="Providers Suportados"
                        secondary="New Relic, Datadog, Grafana"
                      />
                    </ListItem>
                  </List>
                </Box>
      ...
      <Dialog open={openAlertDialog} onClose={() => setOpenAlertDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <FormControl fullWidth className={classes.formField}>
            ...
            <Select
              ...
              onChange={(e) => setNewAlert({ ...newAlert, provider: e.target.value as string })}
            >
        ...
        <DialogActions>
          <Button onClick={handleTestConnections} style={{ marginRight: 'auto' }}>
            🔧 Testar Conexões
          </Button>
