...
export async function createRouter(options: RouterOptions): Promise<express.Router> {
  ...
  router.get('/health', (req, res) => {
    res.json({ status: 'ok', service: 'kenin-duty' });
  });
  ...
  // Testar conexões com providers
  router.post('/providers/test-connections', async (req, res) => {
    try {
      const results = {
        newrelic: {
          status: 'success',
          message: 'Conexão simulada com New Relic',
          timestamp: new Date().toISOString(),
          enabled: config.getOptionalBoolean('keninDuty.providers.newrelic.enabled') || false
        },
        datadog: {
          status: 'success', 
          message: 'Conexão simulada com Datadog',
          timestamp: new Date().toISOString(),
          enabled: config.getOptionalBoolean('keninDuty.providers.datadog.enabled') || false
        },
        grafana: {
          status: 'success',
          message: 'Conexão simulada com Grafana', 
          timestamp: new Date().toISOString(),
          enabled: config.getOptionalBoolean('keninDuty.providers.grafana.enabled') || false
        }
      };

      logger.info('Provider connections tested successfully');
      res.json(results);

    } catch (error) {
      logger.error('Error testing provider connections:', error);
      res.status(500).json({
        error: error.message,
        message: 'Erro ao testar conexões com providers'
      });
    }
  });
