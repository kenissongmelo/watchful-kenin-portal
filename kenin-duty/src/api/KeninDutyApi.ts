...
export class KeninDutyApi {
  ...
  // Provider Accounts API methods
  async getProviderAccounts(type?: string): Promise<any[]> {
    const params = type ? `?type=${type}` : '';
    return this.request<any[]>(`/provider-accounts${params}`);
  }

  async getProviderAccount(id: string): Promise<any> {
    return this.request<any>(`/provider-accounts/${id}`);
  }
  ...
  async createProviderAccount(data: {
    ...
    return this.request<any>('/provider-accounts', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }

  async updateProviderAccount(id: string, data: {
    name?: string;
    enabled?: boolean;
    config?: Record<string, any>;
  }): Promise<any> {
  ...
  async deleteProviderAccount(id: string): Promise<any> {
    return this.request<any>(`/provider-accounts/${id}`, {
      method: 'DELETE',
    });
  }
  ...
  async testProviderConnection(id: string): Promise<any> {
    return this.request<any>(`/provider-accounts/${id}/test-connection`, {
      method: 'POST',
    });
  }

  async getProviderStats(): Promise<any> {
    return this.request<any>('/provider-accounts/stats/by-type');
  }
